    ...
    name = (
      <React.Fragment>
        ...
        <span className={styles.iconSpan}>
          <input className={styles.formField} id='name' type='text' minLength='2' placeholder='Имя' required />
          {svgIcons.name}
        </span>
      </React.Fragment>
    );
  ...
  if (props.phone) {
    phone = (
      <React.Fragment>
        ...
        <span className={styles.iconSpan}>
          <input className={styles.formField} id='phone' type='tel' pattern='7[0-9]{10}' placeholder='Телефон' title='Номер должен содержать 11 цифр и начинатся с 7' required />
        {svgIcons.phone}
        </span>
      </React.Fragment>
    );
